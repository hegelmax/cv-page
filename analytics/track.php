...
if (!$raw || strlen($raw) > 64*1024) { http_response_code(400); echo '{"ok":false}'; exit; }

$payload = json_decode($raw, true);
if (!$payload || !is_array($payload)) { http_response_code(400); echo '{"ok":false}'; exit; }
...
if (strpos($path, '/analytics') === 0) { echo '{"ok":true}'; exit; }
...
$type = in_array(($payload['type'] ?? 'visit'), ['visit','virtual'], true) ? $payload['type'] : 'visit';

// per-IP frequency limiter (simple anti-flood)
$ip   = get_ip();
$now  = (int)(microtime(true)*1000);
$st   = db()->prepare("SELECT ts FROM rate WHERE ip=?");
$st->execute([$ip]);
$prev = (int)($st->fetchColumn() ?: 0);
if ($now - $prev < 300) { echo '{"ok":true}'; exit; } // ≤1 hit per 300ms per IP

db()->prepare("INSERT INTO rate(ip,ts) VALUES(?,?) ON CONFLICT(ip) DO UPDATE SET ts=excluded.ts")->execute([$ip,$now]);

// insert into visits
$sql = "INSERT INTO visits (ts,ip,ua,country,cf_ray,url,path,ref,utm,lang,languages,tz,dpr,vp_w,vp_h,scr_w,scr_h,theme,nav_type,ttfb,dom_inter,dcl,load,type)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
db()->prepare($sql)->execute([
  $ts,$ip,$ua,$cc,$cfr,$url,$path,$ref,$utm,$lang,$lgs,$tz,$dpr,$vp_w,$vp_h,$scr_w,$scr_h,$theme,$nt,$ttfb,$domi,$dcl,$load,$type
]);
